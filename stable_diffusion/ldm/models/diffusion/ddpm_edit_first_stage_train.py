...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
                 ...
                 load_ema=True,
                 *args, **kwargs):
        self.first_stage_train = first_stage_train
    ...
    def shared_step(self, batch, **kwargs):
        if self.first_stage_train:
            x,c,xorgi, xrec =self.get_input(batch, self.first_stage_key, return_first_stage_outputs=True)
        else:
            x, c = self.get_input(batch, self.first_stage_key)
        if self.first_stage_train:
            loss = self(x, c, edited = xorgi, recon= xrec)
        else:
            loss = self(x, c)
        return loss

    def forward(self, x, c, edited=None, recon=None, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        ...
        if edited is not None:
            return self.p_losses(x, c, t, edited=edited, recon=recon, *args, **kwargs)
    ...
    def p_losses(self, x_start, cond, t, edited=None, recon=None, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
